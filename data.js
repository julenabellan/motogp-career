...
// Usamos imágenes de bandera (flagcdn) en vez de emoji: los emoji de bandera
// no se renderizan en todos los sistemas (p. ej. Windows/Chrome a veces los
// muestra como código de país en vez de la bandera).
...
// strength: nivel base del equipo (afecta la simulación de carreras)
// color: color principal del equipo — cámbialo aquí y se actualizará en toda
// la interfaz (tarjeta de piloto, historial de temporadas, etc.) sin tocar
// más código.
const TEAMS = {
  MotoGP: [
  ...
  // Categoría intermedia de la escalera de Superbikes: alimenta a WorldSBK
  // igual que Moto2 alimenta a MotoGP. También sirve como vía lateral para
...
// Iconos SVG en línea para las estadísticas del historial (carreras
// ganadas, podios, poles, abandonos). Usan currentColor / fill:currentColor
// para poder tintarse por CSS sin depender de ninguna librería externa.
const STAT_ICONS = {
...
// Color principal de cada equipo: vive como propiedad `color` en cada
// entrada de TEAMS (arriba). Como algunos nombres de equipo se repiten en
// más de un campeonato (p. ej. "MT Helmets – MSi" en Moto2 y en Moto3),
// se puede pasar el campeonato para desambiguar; si no se encuentra ahí,
// se busca en el resto como respaldo.
